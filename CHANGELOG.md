...
- `npx herdr-picker install-host [--socket] [--extension-id] [--browser-dir]`: copies the host to `~/.config/herdr-picker/`, writes the wrapper script with the absolute node path, registers the host manifest with every Chrome and Chromium profile found (macOS, Linux).
